...
export default function WorkCard({ work, t, acc, movieColor, bookColor, animeColor, currentUser, onAddReview, onDeleteWork, onDeleteReview, onUpdateWork }) {
  ...
  const handleDeleteWork = () => {
    if (window.confirm(`"${work.title}"을 삭제할까요?`)) onDeleteWork(work.id)
  }

  const handleDeleteReview = () => {
    if (window.confirm('내 감상평을 삭제할까요?')) onDeleteReview(work.id)
  }
  ...
  return (
    <div style={{ borderBottom: `0.5px solid ${t.line}` }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, padding: '18px 20px 14px', background: t.bg }}>
        ...
        >
          {work.coverUrl ? (
            ...
            <div style={{ width: '100%', height: '100%', background: pc.bg, color: pc.fg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'DM Serif Display', serif", fontSize: 17, position: 'relative' }}>
          ...
          {isMyWork && <input ref={coverInputRef} type="file" accept="image/*" style={{ display: 'none' }} onChange={handleCoverUpload} />}
        </div>

        {/* Info */}
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 5 }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', fontSize: 10, letterSpacing: '.07em', textTransform: 'uppercase', color: t.muted }}>
            ...
            {isMyWork && (
              <div style={{ display: 'flex', gap: 10 }}>
                <button
                  onClick={() => { setShowEdit(v => !v); setEditTitle(work.title); setEditCreator(work.creator) }}
                ...
                >
                  <svg width="12" height="12" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.6"><path d="M11 2l3 3-9 9H2v-3l9-9z"/></svg>
                  수정
                </button>
                <button
                  ...
                  style={{ background: 'none', border: 'none', color: t.muted, cursor: 'pointer', padding: 0, fontSize: 11, display: 'flex', alignItems: 'center', gap: 3 }}
                >
                  <svg width="12" height="12" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.6"><polyline points="2 4 14 4"/><path d="M5 4V2h6v2"/><path d="M6 7v5M10 7v5"/><rect x="3" y="4" width="10" height="10" rx="1"/></svg>
                  삭제
                </button>
                {isMyWork && !work.coverUrl && (
                  <button
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            ...
            <button onClick={() => setOpen(o => !o)}
              style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, color: t.muted, background: 'none', border: 'none', padding: '3px 0', cursor: 'pointer' }}
            >
              {open ? '닫기' : '평 보기'}
      ...
      {/* 수정 패널 */}
      {showEdit && (
        <div style={{ padding: '14px 20px', borderTop: `0.5px solid ${t.line}`, background: t.bg2 }}>
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
            <button onClick={() => setShowEdit(false)} style={{ padding: '7px 14px', borderRadius: 7, border: `0.5px solid ${t.line2}`, background: 'none', color: t.muted, fontSize: 12, cursor: 'pointer' }}>취소</button>
            <button onClick={handleSaveEdit} style={{ padding: '7px 14px', borderRadius: 7, border: 'none', background: acc.c, color: '#1a1208', fontSize: 12, fontWeight: 500, cursor: 'pointer' }}>저장</button>
          </div>
        </div>
      )}
            ...
            <div key={r.userId} style={{ padding: '13px 20px 13px 88px', borderTop: `0.5px solid ${t.line}` }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                ...
                {r.userId === currentUser?.uid && (
                  <button onClick={handleDeleteReview} style={{ background: 'none', border: 'none', color: t.muted, cursor: 'pointer', padding: 0, fontSize: 11, display: 'flex', alignItems: 'center', gap: 2 }}>
                    <svg width="11" height="11" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.6"><polyline points="2 4 14 4"/><path d="M5 4V2h6v2"/><path d="M6 7v5M10 7v5"/><rect x="3" y="4" width="10" height="10" rx="1"/></svg>
                    삭제
                  </button>
                )}
              </div>
